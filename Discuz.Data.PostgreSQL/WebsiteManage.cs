    ...
    public partial class DataProvider : IDataProvider
    {
        ...
        public int GetHotTopicsCount(int fid, int timeBetween)
        {
            DbParameter[] parms = {
                                        ...
                                        DbHelper.MakeInParam("@timebetween",(DbType)NpgsqlDbType.Integer,4,timeBetween)
							       };
        ...
        public DataTable GetHotTopicsList(int pageSize, int pageIndex, int fid, string showType, int timeBetween)
        {
            DbParameter[] parms = {
                                        ...
                                        DbHelper.MakeInParam("@fid",(DbType)NpgsqlDbType.Integer,4,fid),
                                        DbHelper.MakeInParam("@showtype",(DbType)NpgsqlDbType.Varchar,100,showType),
                                        DbHelper.MakeInParam("@timebetween",(DbType)NpgsqlDbType.Integer,4,timeBetween)
							       };
            ...
            //if (pageIndex == 1)
            //{
            //    commandText = string.Format("SELECT TOP {0} {1},f.[name] FROM [{2}topics] t LEFT JOIN [{2}forums] f ON t.[fid] = f.[fid]  WHERE DATEDIFF(DAY,[postdatetime],NOW())<=7 ORDER BY [replies] DESC,[tid] DESC", pageSize, DbFields.TOPICS_JOIN, BaseConfigs.GetTablePrefix);
            //}
            //else
            //{
            //    commandText = string.Format("SELECT TOP {0} {1},f.[name] FROM [{2}topics] t LEFT JOIN [{2}forums] f ON t.[fid] = f.[fid] WHERE DATEDIFF(DAY,[postdatetime],NOW())<=7 AND [tid] NOT IN (SELECT TOP {3} [tid] FROM [{2}topics] WHERE DATEDIFF(DAY,[postdatetime],NOW())<=7 ORDER BY [replies] DESC,[tid] DESC) ORDER BY [replies] DESC,[tid] DESC",
        ...
        {
            //DbParameter[] parms = {
            //                            DbHelper.MakeInParam("@fidlist",(DbType)NpgsqlDbType.Varchar,3000,fidlist),
            //                            DbHelper.MakeInParam("@count",(DbType)NpgsqlDbType.Integer,4,count),
            //                            DbHelper.MakeInParam("@orderby",(DbType)NpgsqlDbType.Varchar,50,orderby),
            //                            DbHelper.MakeInParam("@continuous",(DbType)NpgsqlDbType.Integer,4,continuous)
            //                       };
            ...
            if (continuous == 1)
            {
                if (fidlist != "")
                {
                    fidlist = "AND [fid] IN(" + fidlist + ") ";

                }
                sb.Append("SELECT  [attach].[aid],[attach].[tid],[attach].[filename],[attach].[attachment],[topic].[title] FROM [dnt_attachments] AS [attach] LEFT JOIN [dnt_topics] AS [topic] ON [attach].[tid] = [topic].[tid] AND [topic].[displayorder]>=0 WHERE aid = (SELECT MIN(aid) from [dnt_attachments] where [width] > 360 AND [height] > 240 and [tid]=[topic].[tid]) " + fidlist + " ORDER BY [attach].[" + @orderby + "] DESC  LIMIT " + count);
            }
            else
            {
                if (fidlist != "")
                {
                    fidlist = "AND [topic].[fid] IN(" + fidlist + ") ";

                }

                sb.Append("SELECT   [attach].[tid],[attach].[filename],[attach].[attachment],[topic].[title] FROM [dnt_attachments] AS [attach] LEFT JOIN [dnt_topics] AS [topic] ON [attach].[tid] = [topic].[tid] AND [topic].[displayorder]>=0  WHERE [attach].[width] > 360  AND [height] > 240 " + fidlist + " ORDER BY [" + orderby + "] DESC  LIMIT " + count);

            }

            return DbHelper.ExecuteDataset(CommandType.Text, sb.ToString()).Tables[0];
        }
    }
}
